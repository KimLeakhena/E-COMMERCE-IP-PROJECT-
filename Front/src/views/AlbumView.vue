    ...
    <body >
      ...
      <div class="w-full flex justify-between items-center ">
        ...
        <ul class="flex items-center gap-6 text-white text-2xl pr-2">
          ...
          <!-- <RouterLink to="/Cart"> -->
            <button v-on:click="navigateTo('cart')" class="flex"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8">
           ...
           <select
            ...
            >
              ...
              <option @click="onSubmit" class="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" v-for="item in items" :key="item.name" :value="item._id">
                <img class="w-4 h-4 mr-2 rounded-full"  :src="categories.imageUrl" >
                {{ categories.name }}
              </option>
            </select>
      </div>
...
export default{
  ...
  methods: {
    ...
    removeProductFromCart(product){
      ...
      // this.$emit("removeProductFromCart",index);
    },
    navigateTo(page){
      this.page=page;
    }
