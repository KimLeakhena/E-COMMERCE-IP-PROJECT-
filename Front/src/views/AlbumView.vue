...
<div class="container flex flex-col flex-wrap content-center justify-center p-4 py-20 mx-auto md:p-10">
  ...
  <div class="flex items-center p-6 space-x-6 rounded-xl ">
    ...
    <div  class="relative  text-gray-600">
            ...
            >
            <option class="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" v-for="item in items" :key="item.name" :value="item._id">
                <!-- <option class="group/item hover:bg-slate-100 ..."> -->
             <img class="h-10 w-10 rounded-full" :src="item.imageUrl" alt="" />
             <div>
              <a @click="onClick(item._id)">{{item.name}}</a>
             </div>
             </option>
                <!-- {{ items.name }}
              </option> -->
